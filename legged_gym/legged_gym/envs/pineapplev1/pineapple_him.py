...
from .pineapple_him_config import PineappleHimRoughCfg

class PineappleHimRough(LeggedRobot):
    cfg : PineappleHimRoughCfg
    ...
    def _init_buffers(self):
        super()._init_buffers()
        self.actuator_net_input = torch.zeros(self.num_envs*self.num_actions, 8, device=self.device, requires_grad=False)
    ...
    def compute_observations(self):
        if self.cfg.commands.num_commands == 5:
            current_obs = torch.cat((   self.commands[:, :3] * self.commands_scale,
                                        ...
                                        (self.dof_pos[:, :3] - self.default_dof_pos[:, :3]) * self.obs_scales.dof_pos,
                                        (self.dof_pos[:, 4:7] - self.default_dof_pos[:, 4:7]) * self.obs_scales.dof_pos,
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        if self.cfg.commands.num_commands == 5:
            ...
            noise_vec[10:16] = noise_scales.dof_pos * noise_level * self.obs_scales.dof_pos
            noise_vec[16:24] = noise_scales.dof_vel * noise_level * self.obs_scales.dof_vel
            noise_vec[24:32] = 0. # previous actions
        else:
            if self.cfg.env.use_lin_vel:
                ...
                noise_vec[26:34] = 0. # previous actions
            else:
                noise_vec[0:3] = 0. # commands
                noise_vec[3:6] = noise_scales.ang_vel * noise_level * self.obs_scales.ang_vel
                noise_vec[6:9] = noise_scales.gravity * noise_level
                noise_vec[9:15] = noise_scales.dof_pos * noise_level * self.obs_scales.dof_pos
                noise_vec[15:23] = noise_scales.dof_vel * noise_level * self.obs_scales.dof_vel
                noise_vec[23:31] = 0. # previous actions
        if self.cfg.terrain.measure_heights:
    ...
    def _compute_torques(self, actions):
        ...
        if self.cfg.control.use_actuator_network:
            ...
            with torch.inference_mode():
                ...
                ), dim=2).view(-1, self.num_actions)   # shape: (num_envs*num_actions, 8)
    ...
    def _reward_nominal_state_hip(self):
        diff = self.dof_pos[:, 0] + self.dof_pos[:, 4]
        return torch.abs(diff)

    def _reward_nominal_state_thigh(self):
        diff = self.dof_pos[:, 1] - self.dof_pos[:, 5]
        return torch.abs(diff)
    
    def _reward_nominal_state_calf(self):
        diff = self.dof_pos[:, 2] - self.dof_pos[:, 6]
        return torch.abs(diff)
    
    def _reward_dof_vel(self):
        # Penalize dof velocities
        return torch.sum(torch.square(self.dof_vel[:, :3]), dim=1) + torch.sum(
            torch.square(self.dof_vel[:, 4:7]), dim=1
        )
    ...
    def _reward_action_smooth(self):
        ...
        return torch.sum(
            torch.square(
                self.actions[:, :3]
                - 2 * self.last_actions[:, :3]
                + self.last_last_actions[:, :3]
            ),
            dim=1,
        ) + torch.sum(
            torch.square(
                self.actions[:, 4:7]
                - 2 * self.last_actions[:, 4:7]
                + self.last_last_actions[:, 4:7]
            ),
            dim=1,
        )
    ...
    def _reward_joint_deviation(self):
        ...
        return torch.sum(torch.abs(self.dof_pos[:, 1:3] - self.default_dof_pos[:, 1:3]), dim=1)+torch.sum(torch.abs(self.dof_pos[:, 5:7] - self.default_dof_pos[:, 5:7]), dim=1)
    
    def _reward_joint_deviation_hip(self):
        # Penalize joint deviation from default position
        return torch.sum(torch.abs(self.dof_pos[:, 0:1] - self.default_dof_pos[:, 0:1]), dim=1)+torch.sum(torch.abs(self.dof_pos[:, 4:5] - self.default_dof_pos[:, 4:5]), dim=1)
